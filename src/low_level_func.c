...
resove_fail:
        if(phostent != NULL)
        {
            device_error("ACS server IP=%s\n", domain_ip);
            strcpy(acs_back_ip, domain_ip);
            if(newurl)
                device_error("ACS new URL=%s\n", newurl);
        }
        else
        {
            if(strcasecmp(acs_back_ip, "") != 0 && new_url)
            {
                *new_url = strdup_check(acs_back_ip);
            }
    
            device_error("ACS domain resolving failed, leave it to libcurl.\n");
        }
    }
    else
    {
